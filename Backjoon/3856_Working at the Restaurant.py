...
def solve(N: int):
    ...
    while idx < N:
        ...
        if cmd == "DROP":
            ...
            # -> 🚨 순서대로 세척해야하기 때문.
            # 만약 p1에 위에서부터 순서대로 [A, B, C] 그릇이 존재했다고 해보자.
            # TAKE 2 명령이 들어오면 A, B가 나가고 C만 남아있게 됨.
            # 여기서 DROP 2 명령이 들어와 p2에 [E, D]가 존재하는데, TAKE 2 명령이 또 들어오게 되면?
            # 무턱대고 MOVE 2->1을 했다간 p1이 [D, E, C] 순서로 배치되는 불상사가 발생하므로, 기존 p1에 있던 [C]를 먼저 빼내고 MOVE 2->1을 실행해야함.
            if p1:
                take_cnt = min(p1, cnt)
